...
export default function AllocationsPage() {
  ...
  useEffect(() => {
    ...
    bedService.getBedsByRoom(selectedRoom).then((res) => {
      ...
      if (list.length > 0) setAllocateForm((f) => ({ ...f, bed_id: list[0].id }));
    });
  }, [selectedRoom]);
    ...
    <div className="animate-fade-in" style={{ maxWidth: '1400px', margin: '0 auto', width: '100%' }}>
      ...
      {showAllocateModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.5)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' }}>
          <div className="animate-pop-in" style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', width: '100%', maxWidth: '520px', maxHeight: '90vh', overflowY: 'auto', padding: '2rem', boxShadow: '0 25px 50px rgba(0,0,0,0.7)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: '1.4rem', fontWeight: '700', color: '#060913' }}>Allocate Bed</h2>
              <button onClick={() => setShowAllocateModal(false)} style={{ background: 'transparent', color: '#060913' }}>
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleAllocateSubmit} style={{ display: 'grid', gap: '1rem' }}>
              ...
              <div>
                ...
                <select required value={allocateForm.bed_id} onChange={(e) => setAllocateForm({ ...allocateForm, bed_id: e.target.value })} style={inputStyle}>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button type="button" onClick={() => setShowAllocateModal(false)} style={{ flex: 1, padding: '0.75rem', background: 'transparent', border: '1px solid #e2e8f0', color: '#060913', borderRadius: '8px', fontWeight: '600' }}>
                  Cancel
                </button>
                <button type="submit" disabled={submitting || !allocateForm.bed_id} style={{ flex: 1, padding: '0.75rem', background: 'linear-gradient(135deg, #ffd369 0%, #faab36 100%)', color: '#060913', borderRadius: '8px', fontWeight: '800', boxShadow: '0 6px 18px rgba(255, 211, 105, 0.35)' }}>
      ...
      {showTransferModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.5)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' }}>
          <div className="animate-pop-in" style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto', padding: '2rem', boxShadow: '0 25px 50px rgba(0,0,0,0.7)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: '1.3rem', fontWeight: '700', color: '#060913' }}>Transfer Resident Bed</h2>
              <button onClick={() => setShowTransferModal(false)} style={{ background: 'transparent', color: '#060913' }}>
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleTransferSubmit} style={{ display: 'grid', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button type="button" onClick={() => setShowTransferModal(false)} style={{ flex: 1, padding: '0.75rem', background: 'transparent', border: '1px solid #e2e8f0', color: '#060913', borderRadius: '8px', fontWeight: '600' }}>
                  Cancel
                </button>
                <button type="submit" disabled={submitting || !transferForm.new_bed_id} style={{ flex: 1, padding: '0.75rem', background: 'linear-gradient(135deg, #ffd369 0%, #faab36 100%)', color: '#060913', borderRadius: '8px', fontWeight: '800', boxShadow: '0 6px 18px rgba(255, 211, 105, 0.35)' }}>
      ...
      {showCheckoutModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15, 23, 42, 0.5)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: '1rem' }}>
          <div className="animate-pop-in" style={{ background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '16px', width: '100%', maxWidth: '440px', maxHeight: '90vh', overflowY: 'auto', padding: '2rem', boxShadow: '0 25px 50px rgba(0,0,0,0.7)' }}>
            <h2 style={{ fontSize: '1.3rem', fontWeight: '700', color: '#060913', marginBottom: '1rem' }}>Check-out Resident</h2>
            ...
            <form onSubmit={handleCheckoutSubmit} style={{ display: 'grid', gap: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                <button type="button" onClick={() => setShowCheckoutModal(false)} style={{ flex: 1, padding: '0.75rem', background: 'transparent', border: '1px solid #e2e8f0', color: '#060913', borderRadius: '8px', fontWeight: '600' }}>
                  Cancel
                </button>
                <button type="submit" disabled={submitting} style={{ flex: 1, padding: '0.75rem', background: '#ef4444', color: '#fff', borderRadius: '8px', fontWeight: '700' }}>
